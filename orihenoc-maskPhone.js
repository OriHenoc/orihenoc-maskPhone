/* 
Ce script étant assez simple à comprendre je n'ai pas jugé utile d'apporter des commentaires à chaque ligne ! 

Enjoy ! */

class ChampPhone {
  constructor(a, b = '(+225)__-________', c = '_') {
  ...
  changeChar(a, b = 1, c = this.mask.length) {
    ...
    if (!this.isNum(d[f]) && !this.isPlaceholder(d[f]))
      do
        if (f += b, f === c) return !1; while (!this.isNum(d[f]) && !this.isPlaceholder(d[f]));
...
document.addEventListener('DOMContentLoaded', function() {
  'use strict';
  let a = document.getElementsByClassName('orihenoc-maskPhone'),
    b = [];
  for (let c = 0; c < a.length; c++) b.push(new ChampPhone(a[c], a[c].dataset.phonemask, a[c].dataset.placeholder))
});
